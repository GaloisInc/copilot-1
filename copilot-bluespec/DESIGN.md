...
2. `mkFibs`, which returns a module of type `Module FibsIfc`. A modules can be
...
`even_action` and `odd_action` methods, respeticely, which print custom
...
(As a side note: the generated Bluespec code alos employs various other tricks
