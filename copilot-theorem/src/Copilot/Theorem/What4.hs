...
--
-- TODO, we should reorganize this proof technique into the API of
-- "Copilot.Theorem.Prove", which allows users more control over what properties
-- are proved and how the proofs are constructed.
module Copilot.Theorem.What4
  ( Solver(..)
  , SatResult(..)
  , prove
...
prove solver spec = do
  ...
  -- TODO We should allow users to specify alternate k values, including 0 (no
  -- induction).
  let proveProperties = forM (CS.specProperties spec) $ \pr -> do
        ...
        base_cases <- forM [0 .. maxBufLen - 1] $ \i -> do
          translateExpr sym mempty (CS.propertyExpr pr)
                        (AbsoluteOffset i) >>= \case
            XBool p -> return p
        ...
        -- TODO, we could determine if the property fails in one of the base
        -- cases (a true model-checking failure) or in the inductive step and
        -- report to the user.
        case solver of
          CVC4 -> liftIO $ WS.runCVC4InOverride sym WS.defaultLogData
                                                clauses $ \case
...
-- necessary to carry out a bisimulation proof.
computeBisimulationProofBundle ::
